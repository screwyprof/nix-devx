...
in
{
  options.perSystem = flake-parts-lib.mkPerSystemOption (
    ...
    in
    {
      ...
      config.devShells = mkIf cfg.enable {
        claude = pkgs.mkShell {
          buildInputs = [
            pkgs.nodejs
          ...
          shellHook = ''
            ...
            export DISABLE_ERROR_REPORTING=''${DISABLE_ERROR_REPORTING:-${
              if cfg.telemetry.disableErrorReporting then "1" else "0"
            }}
            export DISABLE_AUTOUPDATER=''${DISABLE_AUTOUPDATER:-${
              if cfg.telemetry.disableAutoUpdater then "1" else "0"
            }}
            export DISABLE_INSTALLATION_CHECKS=''${DISABLE_INSTALLATION_CHECKS:-${
              if cfg.telemetry.disableInstallationChecks then "1" else "0"
            }}
            export IS_DEMO=''${IS_DEMO:-${if cfg.demo then "1" else "0"}}
            export CLAUDE_CODE_ENABLE_TELEMETRY=''${CLAUDE_CODE_ENABLE_TELEMETRY:-${
              if cfg.telemetry.enableClaudeCodeTelemetry then "1" else "0"
            }}
            export CLAUDE_CODE_DISABLE_FEEDBACK_SURVEY=''${CLAUDE_CODE_DISABLE_FEEDBACK_SURVEY:-${
              if cfg.telemetry.disableFeedbackSurvey then "1" else "0"
            }}
            export CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=''${CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC:-${
              if cfg.telemetry.disableNonessentialTraffic then "1" else "0"
            }}

            export CLAUDE_CODE_SKIP_AUTH_LOGIN=''${CLAUDE_CODE_SKIP_AUTH_LOGIN:-${
              if cfg.ide.skipAuthLogin then "1" else "0"
            }}
            export CLAUDE_CODE_IDE_SKIP_AUTO_INSTALL=''${CLAUDE_CODE_IDE_SKIP_AUTO_INSTALL:-${
              if cfg.ide.skipAutoInstall then "1" else "0"
            }}

            export CLAUDE_BASH_MAINTAIN_PROJECT_WORKING_DIR=''${CLAUDE_BASH_MAINTAIN_PROJECT_WORKING_DIR:-${
              if cfg.bash.maintainProjectWorkingDir then "1" else "0"
            }}
